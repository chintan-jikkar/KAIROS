...
| 14:45 IST / N/A | EOD force-flatten | `ORB_BRK`/`BB_MEANREV` intraday positions must close before illiquid last minutes |
...
- **FX screener isn't on the automated schedule** (see §3); it's on-demand only.
