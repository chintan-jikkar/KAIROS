...
`generate_signal(symbol, df) -> signal | None` and
`should_exit(trade, current_bar) -> (bool, reason)` — so the screener can
assign any qualifying stock to whichever strategy fits its current
volatility/trend regime, and the backtester can replay the *exact* code
live trading uses, not an approximation of it.
...
These are current, honest state — not hidden gaps:
