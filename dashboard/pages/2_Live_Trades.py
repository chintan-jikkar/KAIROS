            ...
            <div class="glass-card" style="margin-bottom:12px;">
                ...
                <div style="display:flex;gap:24px;">
                    ...
                    <div>
                        ...
                        <p class="kairos-mono {pnl_class}" style="font-size:13px;">₹{net_pnl:+,.0f}</p>
                    </div>
                    <div>
                        <p class="kpi-label" style="margin-bottom:2px;">Max loss</p>
                        <p class="kairos-mono negative" style="font-size:13px;">₹{max_loss:,.0f}</p>
                    </div>
                </div>
            </div>
            """,
...
with right_col:
    ...
    today_closed = db.query(Trade).filter(
        func.date(Trade.timestamp_exit) == date.today(), Trade.net_pnl.isnot(None)
    ).all()
    ...
    st.markdown(
        ...
        <div style="display:flex;gap:12px;margin-bottom:16px;">
            <div class="glass-card" style="flex:1;text-align:center;">
                ...
                <p class="kairos-mono {"positive" if realized_today >= 0 else "negative"}" style="font-size:18px;">₹{realized_today:+,.0f}</p>
            </div>
            <div class="glass-card" style="flex:1;text-align:center;">
    ...
    if not open_trades:
        ...
        for t in open_trades:
            rows.append({
                ...
                "Stop": f"₹{t.stop_loss_price:.2f}" if t.stop_loss_price else "–",
                "Target": f"₹{t.target_price:.2f}" if t.target_price else "–",
            })
    ...
    recent_signals = db.query(Signal).order_by(Signal.generated_at.desc()).limit(8).all()
